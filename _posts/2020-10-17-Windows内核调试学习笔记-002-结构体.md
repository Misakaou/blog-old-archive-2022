...
Windows内核的相关的一些学习笔记，这一篇主要是描述EPROCESS，PEB的结构

本文来源：[Moeomu的博客](/posts/Windows内核调试学习笔记-002-结构体/)

## 非公开内核结构

Windows的非公开结构非常多，而有些结构是半公开的，虽然有字段名，但只能推测其用意  
...
每一个进程都由一个EPROCESS结构表示，而这些结构被一个双向链表所连接

### 结构信息

- 0x000偏移是PCB(Process Control Block)的地址，它位于R0中
- 0x0b4偏移是PID，它是标识此进程的唯一标识符
- 0x0b8偏移是活动进程链表，可以用它遍历系统的所有EPROCESS结构
...
- 0x002偏移是是否被调试的FLAG的位置，这个值在R3下即可被修改
- 0x068偏移此值平时是0，被调试时是0x70
- 0x018偏移是_HEAP结构的地址，此结构体内可以判断在偏移0x40=2和0x44=0时为非调试状态
